...
第三方工具的上下文文件默认 **不** 加载，需要在 `megacli.json` 中通过 `compat` 手动启用：

```json
{
  "options": {
    "compat": ["claude", "cursor", "copilot", "gemini", "opencode"]
  }
}
```
...
| `compat` 值 | 启用的路径 |
|---|---|
| `claude` | `CLAUDE.md`、`CLAUDE.local.md`、`.claude/skills/` |
| `cursor` | `.cursorrules`、`.cursor/rules/`、`.cursor/skills/` |
| `copilot` | `.github/copilot-instructions.md` |
| `gemini` | `GEMINI.md`、`gemini.md` |
| `opencode` | `.opencode/skills/`、`.opencode/agents/`、`.opencode/commands/` 及对应的全局目录 |
