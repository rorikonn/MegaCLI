...
import (
	...
	crushlog "github.com/megacli/megacli/internal/log"
	"github.com/megacli/megacli/internal/projects"
	"github.com/megacli/megacli/internal/proto"
	"github.com/megacli/megacli/internal/server"
...
// enabled via the MEGACLI_CLIENT_SERVER environment variable.
func useClientServer() bool {
	v, _ := strconv.ParseBool(os.Getenv("MEGACLI_CLIENT_SERVER"))
	return v
}
...
// MEGACLI_CLIENT_SERVER=1, it connects to a server process and returns a
...
func setupLocalWorkspace(cmd *cobra.Command) (workspace.Workspace, func(), error) {
	...
	if warn := ensureDataDirGitignore(cfg.Options.DataDirectory); warn != "" {
		slog.Warn(warn)
	}
...
func shouldEnableMetrics(cfg *config.Config) bool {
	if v, _ := strconv.ParseBool(os.Getenv("MEGACLI_DISABLE_METRICS")); v {
		return false
	}
...
func createDotCrushDir(dir string) error {
	...
	if warn := ensureDataDirGitignore(dir); warn != "" {
		slog.Warn(warn)
	}
	return nil
}
...
// normalizeLineEndings replaces CRLF with LF for content comparison.
...
// Returns a non-empty warning message if the file could not be written.
func ensureDataDirGitignore(dir string) string {
	...
	case err != nil:
		return fmt.Sprintf("Failed to read .gitignore file: %q %v", gitIgnorePath, err)
	case normalizeLineEndings(string(content)) == normalizeLineEndings(defaultGitIgnore):
		return ""
	}

	if err := os.WriteFile(gitIgnorePath, []byte(defaultGitIgnore), 0o644); err != nil {
		return fmt.Sprintf("Failed to write .gitignore file: %q %v", gitIgnorePath, err)
	}
	return ""
}
