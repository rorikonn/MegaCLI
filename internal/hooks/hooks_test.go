...
func TestBuildEnv(t *testing.T) {
	...
	require.Equal(t, EventPreToolUse, envMap["MEGACLI_EVENT"])
	require.Equal(t, "bash", envMap["MEGACLI_TOOL_NAME"])
	require.Equal(t, "sess-1", envMap["MEGACLI_SESSION_ID"])
	require.Equal(t, "/work", envMap["MEGACLI_CWD"])
	require.Equal(t, "/project", envMap["MEGACLI_PROJECT_DIR"])
	require.Equal(t, "ls", envMap["MEGACLI_TOOL_INPUT_COMMAND"])
	require.Equal(t, "/tmp/f.txt", envMap["MEGACLI_TOOL_INPUT_FILE_PATH"])
}
...
func TestRunnerEnvVarsPropagated(t *testing.T) {
	t.Parallel()
	hookCfg := config.HookConfig{
		Command: `printf '{"decision":"allow","context":"%s"}' "$MEGACLI_TOOL_NAME"`,
	}
	r := NewRunner([]config.HookConfig{hookCfg}, t.TempDir(), t.TempDir())
	result, err := r.Run(context.Background(), EventPreToolUse, "sess", "bash", `{}`)
	require.NoError(t, err)
