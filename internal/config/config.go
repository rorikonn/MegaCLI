...
type Config struct {
	...
	Orchestrator *OrchestratorConfig `json:"orchestrator,omitempty" jsonschema:"description=Multi-agent orchestrator configuration"`
...
type OrchestratorConfig struct {
	Agents []OrchestratorAgentConfig `json:"agents,omitempty" jsonschema:"description=Agent definitions for the orchestrator"`
}

// OrchestratorAgentConfig defines a single agent within the orchestrator.
type OrchestratorAgentConfig struct {
	Name                 string   `json:"name" jsonschema:"description=Unique name for this agent,required"`
	Role                 string   `json:"role" jsonschema:"description=Agent role: primary or secondary,enum=primary,enum=secondary,required"`
	Model                string   `json:"model,omitempty" jsonschema:"description=Model override for this agent (defaults to the global large model)"`
	Tools                []string `json:"tools,omitempty" jsonschema:"description=Allowed tool names for this agent"`
	SystemPromptTemplate string   `json:"system_prompt_template,omitempty" jsonschema:"description=Path to a custom system prompt template"`
}

// IPCConfig controls the inter-process communication server.
type IPCConfig struct {
	Enabled bool `json:"enabled,omitempty" jsonschema:"description=Enable IPC server for cross-instance communication,default=true"`
	Port    int  `json:"port,omitempty" jsonschema:"description=Fixed TCP port for IPC (0 = auto-assign),default=0"`
}
