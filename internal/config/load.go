...
func PushPopMegaCLIEnv() func() {
	var found []string
	for _, ev := range os.Environ() {
		if strings.HasPrefix(ev, "MEGACLI_") {
			pair := strings.SplitN(ev, "=", 2)
			if len(pair) != 2 {
				continue
			}
			found = append(found, strings.TrimPrefix(pair[0], "MEGACLI_"))
		}
	}
	...
	for _, ev := range found {
		os.Setenv(ev, os.Getenv("MEGACLI_"+ev))
	}
...
func (c *Config) setDefaults(workingDir, dataDir string) {
	...
	if str, ok := os.LookupEnv("MEGACLI_DISABLE_DEFAULT_PROVIDERS"); ok {
...
func GlobalConfig() string {
	if crushGlobal := os.Getenv("MEGACLI_GLOBAL_CONFIG"); crushGlobal != "" {
...
func GlobalCacheDir() string {
	if crushCache := os.Getenv("MEGACLI_CACHE_DIR"); crushCache != "" {
		return crushCache
	}
...
func GlobalConfigData() string {
	if crushData := os.Getenv("MEGACLI_GLOBAL_DATA"); crushData != "" {
...
func GlobalSkillsDirs() []string {
	if crushSkills := os.Getenv("MEGACLI_SKILLS_DIR"); crushSkills != "" {
