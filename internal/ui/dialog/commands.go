...
func (c *Commands) setCommandItems(commandType CommandType) {
	...
	case UserCommands:
		for _, cmd := range c.customCommands {
			...
			commandItems = append(commandItems, NewCommandItem(c.com.Styles, "custom_"+cmd.ID, cmd.Name, cmd.Description, action))
		}
	case MCPPrompts:
...
func (c *Commands) defaultCommands() []*CommandItem {
	commands := []*CommandItem{
		...
		NewCommandItem(c.com.Styles, "switch_agent", "Switch Agent", "ctrl+a", ActionOpenDialog{AgentsID}),
	}

	// Only show these commands if there's an active session
	if c.hasSession {
