...
func TestCrushIgnore(t *testing.T) {
	...
	// Create a .megacliignore file that ignores .log files
	require.NoError(t, os.WriteFile(".megacliignore", []byte("*.log\n"), 0o644))
...
func TestShouldExcludeFile(t *testing.T) {
	...
	require.True(t, ShouldExcludeFile(tempDir, customIgnored), "Expected custom_ignored to be ignored by .megacliignore")
...
func TestShouldExcludeFileHierarchical(t *testing.T) {
	...
	// Test hierarchical ignore behavior - this should work because the .megacliignore is in the parent directory
	require.True(t, ShouldExcludeFile(tempDir, nestedNormal), "Expected normal_nested to be ignored by subdir .megacliignore")
