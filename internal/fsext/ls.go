...
// megacliGlobalIgnorePatterns returns patterns from the user's
// ~/.config/megacli/ignore file.
var megacliGlobalIgnorePatterns = sync.OnceValue(func() []gitignore.Pattern {
	name := filepath.Join(home.Config(), "megacli", "ignore")
	bts, err := os.ReadFile(name)
	if err != nil {
		if !os.IsNotExist(err) {
			slog.Debug("Failed to read megacli global ignore file", "path", name, "error", err)
		}
		return nil
	}
...
func (dl *directoryLister) getDirPatterns(dir string) []gitignore.Pattern {
	return dl.dirPatterns.GetOrSet(dir, func() []gitignore.Pattern {
		...
		for _, ignoreFile := range []string{".gitignore", ".megacliignore"} {
...
func (dl *directoryLister) getCombinedMatcher(dir string) gitignore.Matcher {
	return dl.combinedMatchers.GetOrSet(dir, func() gitignore.Matcher {
		...
		// Add global ignore patterns (git core.excludesFile + megacli global ignore).
		allPatterns = append(allPatterns, gitGlobalIgnorePatterns()...)
		allPatterns = append(allPatterns, megacliGlobalIgnorePatterns()...)
