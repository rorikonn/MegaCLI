...
func TestTracker_BuiltinSkillTracking(t *testing.T) {
	...
	// Simulate active skills including a builtin skill (megacli-config).
	activeSkills := []*Skill{
		{Name: "megacli-config", Description: "MegaCLI config", Builtin: true},
		{Name: "go-doc", Description: "Go docs", Builtin: false},
	}
	tracker := NewTracker(activeSkills)

	// Initially not loaded.
	require.False(t, tracker.IsLoaded("megacli-config"))
	require.False(t, tracker.IsLoaded("go-doc"))

	// Mark builtin skill as loaded (simulating read via crush://...).
	tracker.MarkLoaded("megacli-config")
	require.True(t, tracker.IsLoaded("megacli-config"))
