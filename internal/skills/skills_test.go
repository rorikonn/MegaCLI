...
func TestToPromptXMLBuiltinType(t *testing.T) {
	...
	skills := []*Skill{
		...
		{Name: "user-skill", Description: "A user skill.", SkillFilePath: "/home/user/.config/megacli/skills/user-skill/SKILL.md"},
	}
	xml := ToPromptXML(skills)
...
func TestDiscoverBuiltin(t *testing.T) {
	...
	var foundJQ bool
	for _, s := range discovered {
		if s.Name == "json-processor" {
			foundJQ = true
			require.Equal(t, "crush://skills/json-processor/SKILL.md", s.SkillFilePath)
			require.Equal(t, "crush://skills/json-processor", s.Path)
			require.NotEmpty(t, s.Description)
			require.NotEmpty(t, s.Instructions)
			require.True(t, s.Builtin)
		}
	}
	require.True(t, foundJQ, "json-processor builtin skill not found")
